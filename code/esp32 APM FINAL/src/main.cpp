...
// code voor ultrasonic sensor
// int readUltrasonicLinks() {
...
//   long duur = pulseIn(echoLinks, HIGH);
...
//     if (sr_irAchter || sr_irLinks || sr_irRechts) return false;
...
bool safetyDelay(unsigned long ms) {
  ...
  while (millis() - startTijd < ms) {
    ...
    readShiftInputs();   // update IR sensoren

    // stop delay als een sensor iets ziet
    if (sr_irLinks || sr_irRechts || sr_irAchter) return false;
...
void obstakelVermijding() {
  ...
  int afstandLinks  = distanceL;
  int afstandRechts = distanceR;

  bool irLinks  = sr_irLinks;
  bool irRechts = sr_irRechts;
  bool irAchter = sr_irAchter;

  // obstakel links
  if (irLinks || afstandLinks < 20) {
    backwards(200);
    safetyDelay(200);
    right(200);
    safetyDelay(300);
  ...
  // obstakel rechts
  else if (irRechts || afstandRechts < 20) {
    backwards(200);
    safetyDelay(200);
    left(200);
    safetyDelay(300);
